...
def train_test_split (dataset: Dataset, test_size: float = 0.2, random_state: int = 42) -> Tuple[Dataset, Dataset]:
    ...
    """
    # set random state
    np.random.seed(random_state) # adicionar sempre isto para não obter coisas aleatórias e poder reproduzir esta
    # pipeline vezes sem conta sempre com o mesmo resultado
    ...
    train = Dataset(X=dataset.X[train_idxs], y=dataset.y[train_idxs], features_names=dataset.features_names,
                    label_names=dataset.label_names)
    test = Dataset(X=dataset.X[test_idxs], y=dataset.y[test_idxs], features_names=dataset.features_names,
                   label_names=dataset.label_names)
    return train, test
